...
const ConversationPage = () => {
  ...
  const [messages, setMessges] = useState<ChatCompletionRequestMessage[]>([]);
  ...
  const onSumit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      setMessges((current) => [...current, userMessage, response.data]);
      form.reset();
    } catch (error: any) {
      // too pro model
      console.log(error);
  ...
  return (
    <div>
      <Heading
        title="Conversation"
        description="Ourt most advanced converation model"
        icon={MessageSquare}
